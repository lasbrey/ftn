...
const DepositModal = ({ selectedDeposit, isModalOpen, setIsModalOpen }) => {
    const handleApprove = async () => {
        try {
            ...
            const balanceQuery = query(balanceRef, where("userId", "==", selectedDeposit?.userId));
            ...
            const DepositQuery = query(depositRef, where("depositId", "==", selectedDeposit?.depositId));
                        ...
                        <div className="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                            <div className="border-b border-stroke py-4">
                                <h3 className="font-medium text-black px-6.5 py-2 text-xl border-b border-stroke">
                                    Pending deposit from: <span className='font-bold'>{selectedDeposit?.name}</span>
                                </h3>
                                <div>
